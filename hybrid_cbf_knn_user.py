...
"""
This one is a knn on the users
"""
# the dict is {user: {user: sim}}
knn = {}
...
with open('submission/cbf_knn_hybrid_merge_w0knn_w0cbf.csv', 'w', newline='') as f:
    ...
    for i in range(1, len(user_test_list)):
        ...
        my_dict['testItems'] = cbf_recommendations(urm[int(user_test_list[i][0])],int(user_test_list[i][0]), icm, knn, urm, shrink=10, sim_skr=20, w_cbf=0.8, w_knn=0.2)
        w.writerow(my_dict)
