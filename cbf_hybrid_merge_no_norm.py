...
def cbf_recommendations(user_ratings,user, icm_m, knn, sim_skr=20, shrink=10, w_cbf=0.5, w_knn=0.5):
    ...
    * TODO:
    ...
    for movie in totals_cbf:
        rankings.setdefault(movie, totals_cbf[movie] * w_cbf)
    ...
    for movie in totals_knn:
        if movie not in rankings:
            rankings.setdefault(movie, totals_knn[movie] * w_knn)
...
with open('submission/cbf_knn_hybrid_merge.csv', 'w', newline='') as f:
    ...
    for i in range(1, len(user_test_list)):
        ...
        my_dict['testItems'] = cbf_recommendations(urm[int(user_test_list[i][0])], int(user_test_list[i][0]), icm, knn, shrink=10, sim_skr=10, w_cbf=0.6, w_knn=0.4)
        w.writerow(my_dict)
